...
def scale_3d_data_channels(X_train, X_val, X_test=None):
    """
    ...
    for c in range(X_train.shape[1]):
        ...
        if X_test is not None:
            X_test_scaled[:, c, :] = (X_test[:, c, :] - mean) / std
...
def train_and_evaluate_simple_cnn(X_train, y_train, X_val, y_val, epochs=25, batch_size=32, lr=0.001, device='cuda', verbose=False):
    """
    Handles the training loop for the SimpleCNNBaseline network.
    Returns the trained model instance for downstream evaluation.
    """
...
def run_ml_loso_universal(X_features, y_all, subject_all, model_factory, seed=42, exp_name="ml_loso"):
    ...
    UPDATED: Strict 1 Test Subject, 1 Validation Subject, Remaining as Train.
    """
    set_seed(seed)
    ...
    print(f"[INFO] Protocol: 1 Test Subject, 1 Validation Subject, N-2 Training Subjects")
    
    for fold_idx, test_subject in enumerate(subjects, start=1):
        # Select Validation Subject (e.g., the "next" subject in the array to ensure rotation)
        test_idx_in_arr = np.where(subjects == test_subject)[0][0]
        val_subject = subjects[(test_idx_in_arr + 1) % len(subjects)]
        
        test_mask = (subject_all == test_subject)
        val_mask = (subject_all == val_subject)
        train_mask = ~(test_mask | val_mask)
        
        X_train, y_train = X_features[train_mask], y_all[train_mask]
        X_val, y_val = X_features[val_mask], y_all[val_mask]
        ...
        # Fit model on training subjects only
        model.fit(X_train_scaled, y_train)
        
        # Evaluate Validation Subject
        ...
        cm = confusion_matrix(y_test, y_test_pred, labels=list(range(10)))
        global_cm += cm
        ...
        results.append({
            ...
            "val_percent": round(len(X_val) / total_samples * 100, 2),
        ...
        print(f"Fold {fold_idx}/{len(subjects)} | Val: {val_subject} | Test: {test_subject} | Test Acc: {test_acc:.4f} | Test F1: {test_f1:.4f}")
...
def run_simple_cnn_within_subject_universal_logged(X_raw, y_all, subject_all, n_splits=5, epochs=25, seed=42, exp_name="cnn_within"):
    ...
    for subj in subjects:
        ...
        for fold_id, (train_idx, val_idx) in enumerate(skf.split(X_subj, y_subj), start=1):
            ...
            model = train_and_evaluate_simple_cnn(X_train_s, y_train, X_val_s, y_val, epochs=epochs, device=device, verbose=False)
            
            model.eval()
            with torch.no_grad():
                val_outputs = model(torch.FloatTensor(X_val_s).to(device))
                y_pred = val_outputs.argmax(dim=1).cpu().numpy()
...
def run_simple_cnn_loso_universal_logged(X_raw, y_all, subject_all, epochs=25, seed=42, exp_name="cnn_loso"):
    ...
    for fold_idx, test_subject in enumerate(subjects, start=1):
        ...
        X_train_s, X_val_s, X_test_s = scale_3d_data_channels(X_train, X_val, X_test)
        
        # Train model using Validation set for monitoring/early-stopping
        model = train_and_evaluate_simple_cnn(X_train_s, y_train, X_val_s, y_val, epochs=epochs, device=device, verbose=True)
        
        model.eval()
        with torch.no_grad():
            # Evaluation on Validation Subject
            val_outputs = model(torch.FloatTensor(X_val_s).to(device))
            y_val_pred = val_outputs.argmax(dim=1).cpu().numpy()
            val_acc = accuracy_score(y_val, y_val_pred)
            val_f1 = f1_score(y_val, y_val_pred, average="macro")
            
            # Evaluation on Test Subject
            test_outputs = model(torch.FloatTensor(X_test_s).to(device))
            y_test_pred = test_outputs.argmax(dim=1).cpu().numpy()
            test_acc = accuracy_score(y_test, y_test_pred)
            test_f1 = f1_score(y_test, y_test_pred, average="macro")

        global_cm += confusion_matrix(y_test, y_test_pred, labels=list(range(10)))
        ...
        print(f"Fold {fold_idx} Result -> Val: {val_subject} | Test: {test_subject} | Final Test Acc: {test_acc:.4f} | F1: {test_f1:.4f}")
